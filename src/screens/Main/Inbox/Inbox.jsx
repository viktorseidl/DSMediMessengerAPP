...
export default function Inbox ({ navigation }) {
        ...
        searchQuery === "" || Object.values(item).toString().toLowerCase().includes(searchQuery.toLowerCase()) ?
            ...
            <Pressable style={({pressed})=> [styles.emailItem,{opacity:pressed?0.5:1}]} onPress={()=>navigation.navigate("ShowMessage", {item: item})}>
                ...
                <View style={styles.emailContent}>
                    <Text style={styles.Erfasser}>{item.fullName!==null && item.fullName!==""? item.fullName:item.Sender}</Text>
                    <Text style={styles.Bezeichnung}>{item.Betreff}</Text>
    ...
    async function loadMessages() {
        ...
        try {
            const data = await apiConnect({"user": user.username, "password": user.password, "dbtype": user.dbtype}, "messages/1")
    ...
    return (
         ...
         <Main title="Posteingang" searchQuery={searchQuery} setSearchQuery={(query)=>setSearchQuery(query)}>
                {/* Email list */}
                {!mailsIsLoaded?
